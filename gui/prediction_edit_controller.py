"""
Controller for drag-move / corner-resize editing of prediction boxes.
"""
...
from .annotation_edit_controller import HANDLE_RADIUS, MIN_BOX_SIDE_PX
...
class PredictionEditController:
    def __init__(
        self,
        *,
        get_selected_prediction_index: Callable[[], int | None],
        get_prediction_canvas_rect: Callable[[int], tuple[float, float, float, float] | None],
        get_canvas: Callable[[], object | None],
        image_canvas: object,
        render_canvas: Callable[[int | None], None],
        on_request_update_prediction: Callable[[int, int, int, int, int], bool],
        on_restore_preview: Callable[[int], None],
        on_canvas_updated: Callable[[], None] | None = None,
    ) -> None:
        self._get_selected_prediction_index = get_selected_prediction_index
        self._get_prediction_canvas_rect = get_prediction_canvas_rect
        self._get_canvas = get_canvas
        self._image_canvas = image_canvas
        self._render_canvas = render_canvas
        self._on_request_update_prediction = on_request_update_prediction
    ...
    def handle_press(self, event) -> bool:
        pred_index = self._get_selected_prediction_index()
        if pred_index is None:
            return False
        rect = self._get_prediction_canvas_rect(pred_index)
        if rect is None:
            return False
        ...
        self._render_drag_preview(self._rect_for_pointer(self._drag, mx, my), pred_index=pred_index)
        return True
    ...
    def handle_move(self, event) -> bool:
        ...
        self._render_drag_preview(
            self._rect_for_pointer(self._drag, mx, my),
            pred_index=self._drag.pred_index,
        )
        return True
    ...
    def handle_release(self, event) -> bool:
        ...
        if right - left < MIN_BOX_SIDE_PX or bottom - top < MIN_BOX_SIDE_PX:
            self._restore_preview(drag.pred_index)
            return False

        ok = self._on_request_update_prediction(
            drag.pred_index,
            int(round(left)),
            int(round(top)),
            int(round(right)),
        ...
        self._on_restore_preview(drag.pred_index)
        return True
    ...
    def cancel_active_drag(self) -> None:
        ...
        pred_index = self._drag.pred_index
        self._drag = None
        self._restore_preview(pred_index)
    ...
    def _render_drag_preview(
        ...
        *,
        ...
        self._render_canvas(pred_index)
        canvas = self._get_canvas()
        if canvas is None:
            return
